...
userRouter.delete("/:userId/block", async (req, res) => {
  try {
    const blockerId = req.userId;
    const blockedId = req.params.userId;
    const result = await BlockedUser.findOneAndDelete({ blocker: blockerId, blocked: blockedId });
    if (!result) {
...
messageRouter.get("/:peerId", async (req, res) => {
  try {
    ...
    const blocked = await BlockedUser.findOne({
      $or: [{ blocker: me, blocked: peer }, { blocker: peer, blocked: me }]
    });
    if (blocked) {
...
messageRouter.post("/:peerId/read", async (req, res) => {
  try {
    const me = req.userId;
    const peer = req.params.peerId;
    const result = await Message.updateMany({ from: peer, to: me, readAt: null }, { $set: { readAt: new Date() } });
...
messageRouter.delete("/:peerId", async (req, res) => {
  try {
    ...
    return res.json({ message: "Chat history cleared", deletedCount: result.deletedCount });
  } catch (err) {
...
messageRouter.get("/unread/count", async (req, res) => {
  try {
    ...
    const unreadCounts = await Message.aggregate([
      { $match: { to: new mongoose.Types.ObjectId(userId), readAt: null } },
...
// Broadcast presence function
async function broadcastPresence(userId, online, io, User) {
  try {
...
export const startServer = async () => {
  ...
  io.on("connection", (socket) => {
    ...
    socket.join(userId);
    broadcastPresence(userId, true, io, User);
    ...
    socket.on("chat:send", async ({ to, content }, ack) => {
      try {
        ...
        const blocked = await BlockedUser.findOne({
          $or: [{ blocker: userId, blocked: to }, { blocker: to, blocked: userId }]
        });
        if (blocked) {
          if (ack) ack({ ok: false, error: "USER_BLOCKED" });
          return;
        }
        const msg = await Message.create({ from: userId, to, content: content.trim(), deliveredAt: new Date() });
        const populatedMsg = await Message.findById(msg._id).populate('from', 'name avatar').populate('to', 'name avatar');
    ...
    socket.on("chat:read", async ({ messageIds }) => {
      try {
        ...
        await Message.updateMany({ _id: { $in: messageIds }, to: userId, readAt: null }, { $set: { readAt: new Date() } });
        const messages = await Message.find({ _id: { $in: messageIds } });
        messages.forEach(msg => {
          io.to(msg.from.toString()).emit("chat:read", { messageId: msg._id, readAt: new Date(), readBy: userId });
        });
      } catch (err) {
    ...
    socket.on("disconnect", (reason) => {
      ...
      if (userSockets) {
        ...
        if (userSockets.size === 0) {
          ...
          broadcastPresence(userId, false, io, User);
        }
      }
      if (typingUsers.has(userId)) {
